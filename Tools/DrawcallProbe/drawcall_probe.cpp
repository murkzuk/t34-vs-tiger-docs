...
typedef HRESULT (STDMETHODCALLTYPE *SetTransformFn)(
...
static SetTransformFn     g_orig_settransform;
static double g_fov, g_zn, g_zf;   // live frustum, printed in every report
...
static void report(double wall_ms)
{
  ...
  // The frustum in force for THIS block. Printed every time rather than only
  // on change, so a run still yields the numbers even if change-detection
  // misbehaves - one 5 s block looking externally, one down the gunsight, and
  // the pair can be compared directly against the draw-call counts below.
  if (g_zf > 0.0)
    llog("  FRUSTUM NOW: fov %.2f deg,  znear %.3f m,  zfar %.1f m,  depth ratio %.0f:1",
         g_fov, g_zn, g_zf, g_zf / (g_zn > 0 ? g_zn : 1));
  else
    llog("  FRUSTUM NOW: not seen yet - SetTransform hook is NOT reporting");
  llog("  PER FRAME");
...
// ---------------------------------------------------------------------------
// PROJECTION MATRIX - the only place the TRUE near/far planes and field of
// view exist.  [jm 2026-08-27]
//
// Camera.script declares ZFar 500 in REDUX and 6437 in ZW, but the user can
// plainly see further than 500 m in REDUX, so the declared value is NOT what
// reaches the GPU. Reading the matrix settles it without guessing: D3D builds
// a left-handed perspective projection as
//
//     _22 = cot(fovY/2)
//     _33 = zf / (zf - zn)
//     _43 = -zn * zf / (zf - zn)
//
// so zn = -_43/_33 and zf = _33*zn/(_33 - 1), exactly, and fovY = 2*atan(1/_22).
//
// This is the measurement that answers "why is the gunsight cheaper in one
// build and ruinous in the other" - it reports the real frustum for whichever
// view is on screen, so switching to the gunsight in game prints its own line.
//
// Logged only when the frustum CHANGES, because SetTransform is called for
// every object's world matrix and we filter to PROJECTION alone.
// ---------------------------------------------------------------------------
static HRESULT STDMETHODCALLTYPE HookSetTransform(
    IDirect3DDevice9 *self, D3DTRANSFORMSTATETYPE state, const D3DMATRIX *m)
{
  if (state == D3DTS_PROJECTION && m) {
    float q = m->_33, r43 = m->_43, yscale = m->_22;
    // [jm 2026-08-27] The threshold here was `q > 1.0001f`, which is wrong and
    // would have silently skipped the very case this exists to measure.
    // q = zf/(zf-zn), so a NEAR plane of 0.1 against a far plane of 6437 gives
    // q = 1.0000155 - under that threshold, and the gunsight is exactly where
    // the near plane gets small. Compare against 1.0 with a tiny epsilon, and
    // guard the (q-1) division separately.
    double qd = (double)q;
    if (qd > 1.0 && (qd - 1.0) > 1e-12 && yscale > 0.0001f) {
      double zn = -(double)r43 / qd;
      double zf = qd * zn / (qd - 1.0);
      double fov = 2.0 * atan(1.0 / (double)yscale) * 57.29577951308232;
      g_fov = fov; g_zn = zn; g_zf = zf;   // always current
      static double l_zn, l_zf, l_fov;
      static int seen;
      // 2% band: the engine jitters the matrix slightly frame to frame.
      bool changed = !seen ||
                     fabs(zf - l_zf) > l_zf * 0.02 ||
                     fabs(zn - l_zn) > l_zn * 0.02 ||
                     fabs(fov - l_fov) > 0.5;
      if (changed) {
        l_zn = zn; l_zf = zf; l_fov = fov; seen = 1;
        llog_try("FRUSTUM: fov %6.2f deg   znear %8.3f m   zfar %9.1f m   "
                 "depth ratio %.0f:1", fov, zn, zf, zf / (zn > 0 ? zn : 1));
      }
    }
  }
  return g_orig_settransform(self, state, m);
}
...
static void reapply_device_hooks(bool quiet)
{
  ...
  patch_slot(vt,  44, (void *)HookSetTransform, (void **)&g_orig_settransform, "SetTransform");
  ...
  // Prove it rather than assume it. Two runs were wasted on a hook that built
  // fine and never installed, because this list had a hand-copied twin.
  llog(vt[44] == (void *)HookSetTransform
         ? "SetTransform hook VERIFIED in slot 44 - frustum reporting is live"
         : "*** SetTransform hook NOT INSTALLED in slot 44 - no frustum data ***");
}
    ...
    D3DPRESENT_PARAMETERS *pp, IDirect3DDevice9 **ppdev)
{
  ...
  if (new_vt) {
    ...
    // [jm 2026-08-27] This used to be a SECOND, hand-copied list of the same
    // nine patch_slot calls. Adding a tenth hook (SetTransform) to
    // reapply_device_hooks alone therefore built cleanly, logged nothing, and
    // silently never installed - because THIS is the path that runs at
    // startup and reapply only runs on device Reset. Two runs were wasted
    // before the log showed slot 44 missing between 26 and 57.
    // One list now, so a hook added in one place cannot go missing here.
