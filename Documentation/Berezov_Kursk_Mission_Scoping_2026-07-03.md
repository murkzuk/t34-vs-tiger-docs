...
## Sixth in-Editor test — router-zone fix wasn't enough either; found the real bug: scheduling from the wrong object

User tested again after the router-zone fix and reported still-total stationariness (fresh, short `editor.log`, no compile/runtime errors, no router-related failures this time either - just genuinely nothing happening). The router-zone fix was real and necessary (confirmed against `Campaign_2\Mission_5`'s own populated zone), but evidently not sufficient on its own.

Went back to `Campaign_2\Mission_5`'s actual working code for a second, more careful look at *where* its scripted advances get scheduled from - not just what they call. Found the real pattern: `Mission.script`'s own `event void StartCombat()` dispatches every group's first move order itself, by name, e.g. `sendEvent(0.0, "CC2M5Group01T_34_85", "StartFirstAdvance", [])` - sent **from the Mission object to the group by its Content.script object ID string**, not scheduled by the group calling `sendEvent` on itself. `StartCombat()` in turn gets called once from `StartMission()` (via `StartFirst()`/`StartOutPlayer()`). Also noticed every one of Mission_5's sendEvent-dispatched target methods is declared `event void MethodName()`, not plain `void` - this session's original `StartAdvance()`/`TriggerAdvance()` methods were plain `void`, which likely made them invisible to the dispatch mechanism entirely.

This matches this pass's actual bug precisely: the previous pass had each `BerezovXxx` group class call `sendEvent(delay, getIdentificator(this), "StartAdvance", [])` **on itself, from within its own `Init()`** - almost certainly a no-op, since a `UnitGroup` object is unlikely to be fully registered in the event-dispatch system at the exact moment its own `Init()` is still running, and the target methods weren't declared with the required `event` keyword either.

**Real fix**: removed all self-scheduling from `MissionTasks.script` - each group now just exposes a plain `event void StartAdvance()` (no `Init()`-time scheduling at all). `Mission.script` now does all the dispatching itself, exactly mirroring the proven pattern: the four unconditional advances (KG Kaiser, Zug Weidinger, Plt.Popov, Plt.Zhilin) are sent from `StartMission()` via `sendEvent(delay, "GroupID", "StartAdvance", [])`; the two destroy-triggered advances (Zug Lex, Plt.Oleshev) are sent the same way from inside the existing `OnObjectDestroyed` kill-list-zero branches, replacing the old `GetObject(...).TriggerAdvance()` calls.

Verified balance on both files (`MissionTasks.script`: 44/44, 106/106, 25/25; `Mission.script`: 13/13, 79/79, 32/32) and mirrored to the docs repo. Not yet re-tested - this is a genuinely different, better-grounded fix than the router-zone one, and should be the one that actually gets units moving.

## Seventh in-Editor test — dispatch fix confirmed working, but hit a router A* failure; deep-dived and still unresolved

Great news first: the console screenshot the user sent (live from the Editor's own play-test) proves the previous fix worked - `Berezov_KGKaiser_1` genuinely received and attempted its `StartAdvance` move order this time (`SetOrder_MoveToEx` fired, a real `[Router] order_MoveTo` entry appears in the log). So the "schedule from the Mission, not from the group's own Init()" fix from the previous section is validated.

But the move itself fails: `[Router] Warning! Path Generated in more than 20000 iterations: - From: [66, 782] - To: [-1, -1] AStarSteps = 20000`, followed by `[CBaseAITask::OnUnreacheable] Unit with ID=Berezov_KGKaiser_1 can not move to _Destination=Vector(576.xx, 6884.xx, 578.14)` - repeating every retry with the destination drifting by a few centimeters each time (the unit jittering in place while the router keeps failing).

Ruled out, with real evidence, three plausible causes before reaching for another guess:
1. **Terrain passability along the whole route** - sampled `RouterZone_Test.bmp` (2048x2048, indexed/palette mode, confirmed the pixel value *is* the `ZMC_*` zone code directly - `Common\BaseZone.script`'s constants match observed values exactly) at 51 points along the straight line from KG Kaiser's spawn to the Berezov village target: every single sample came back `Forest01`/`Forest03`/`Grass01`/`OffRoad01` - all explicitly listed in `CBaseRouterMap`'s own `PassableZones` array in `Common\RouterMap.script`. No water, no `NonPassable`, no unlisted codes anywhere on the route.
2. **Terrain height mismatch** - the flat `Z=610.0` used for all group positions and move targets was checked against the real heightmap value at these exact points (reusing this session's established `Z = 0.010522*raw + 523.43` linear fit): real height comes out to ~608.1-608.6 across all four key points (KG Kaiser spawn, Berezov, Gonki, player spawn) - a ~2-unit difference, negligible.
3. **Missing precomputed router-graph asset** - checked whether any mission (including the real, working `Campaign_2\Mission_5`) ships a separate baked navigation-graph file alongside its zone bitmap. None do - `CRouterPrecalculatedGraph::OnGeneratePrecalculatedGraph()` in `Common\Router.script` calls `GenerateGraph()` live, at load time, from the zone bitmap + `RouterWorkingZones` alone (matches the very fast, ~0.03s "RouterMap generation time" log line - not a red flag, just how this system always works).

With those three ruled out, the remaining live suspect is the **format of `RouterWorkingZones` itself**: it's an unlabeled 4-number array, and the one real working reference (`Mission_5`'s `[40000, 40000, 60000, 60000]`) is unfortunately *symmetric* between its two coordinate pairs - it can't actually disambiguate axis order (`[minX,minY,maxX,maxY]` vs. any other reading produces the identical box for a square input). This was a real gap in the confidence expressed in the previous section's fix.

**Fix attempted**: widened `RouterWorkingZones` from `[0.0, 3000.0, 8000.0, 8000.0]` to `[0.0, 0.0, 18000.0, 18000.0]` - the entire map, symmetric and axis-order-safe by construction, removing zone coverage as a variable entirely regardless of how the four numbers are actually read.

**Honest caveat, not overstated as a confirmed fix this time**: unlike the previous two rounds, this one is not backed by a clear, specific root-cause confirmation - the passability, height, and asset checks all came back clean, so this is closing off the one remaining plausible lead found through static analysis, not a diagnosed-and-proven bug. If this doesn't resolve it, the likely next step is a control experiment: check whether *any* unit, in *any* mission, can be given a manual move order successfully on this exact reused `SteppeTemplate` terrain data at all - to establish whether router movement works on this terrain in principle, independent of anything Berezov-specific.

Verified balance (13/13, 81/81, 32/32) and mirrored to the docs repo.

## Eighth in-Editor test — real progress: 3 of 4 timed groups moved cleanly; KG Kaiser isolated and fixed

User tested again with a much longer session (32875-line log). Genuinely good news buried under "poor at best": grepping every mention of each scripted group's name showed **Zug Weidinger, Plt. Popov, and Plt. Zhilin all got clean `SetOrder_MoveTo` dispatches with zero router errors anywhere in the log** - the dispatch-from-Mission fix from the previous section is fully validated for single-point moves. Zug Lex and Plt. Oleshev showed no `SetOrder_MoveTo` at all, but that's expected and correct: they're destroy-triggered (on Plt. Samsonov / Plt. Kutuzov being wiped out), and nothing in the log suggests either kill list emptied this session.

**KG Kaiser was the sole broken case** - not "no unit moves," but two of its four tanks (`Berezov_KGKaiser_1`/`_2`, not `_3`/`_4`) stuck in an unreachable-path retry loop (~3,450 combined failed attempts across the session, `AStarSteps=20000` every time, `_Destination` always just a few meters from the tank's own spawn rather than either real target). Ruled out several hypotheses with real evidence before finding the actual cause:
- Sampled local terrain around all 4 tanks' spawns (+-40 units): identical zone classification for all four, so not a terrain-type difference between the two that failed and the two that didn't.
- **User visually confirmed in the Editor** (screenshot) that the stuck tank sits in open, flat, empty grassland - no trees, no obstacles nearby at all. This directly ruled out "boxed in by procedurally-placed forest objects," the leading hypothesis after the terrain-sampling checks came back clean. The screenshot's Properties panel also showed the tank's own `Route` property as a genuinely empty array - confirming the router really did produce zero waypoints, not just failing to report a real one.
- With physical obstruction ruled out, re-examined what was structurally different about KG Kaiser's own order versus the three that worked: **KG Kaiser was the only group given a *2-point* array in `SetOrder_MoveToEx` (Berezov, then Gonki)** - every working group used a single-point array. Reading `SetOrder_MoveToEx`'s own implementation in `Common\UnitGroup.script` confirmed a likely mechanism: it tags every queued leg with the **literal, non-unique string `"MoveToEx"`** regardless of array index, so a 2-point call queues two orders sharing an identical name - a very plausible way for the queue/order-completion tracking to get confused when more than one point is queued at once.

**Fix**: split KG Kaiser's route into two separate single-point calls instead of one 2-point array - `StartAdvance()` (Berezov leg) and a new `StartAdvance2()` (Gonki leg), each a lone-element `SetOrder_MoveToEx` call, matching the exact pattern already proven to work for the other three groups. Mission.script now dispatches both legs separately: `sendEvent(60.0, "BerezovKGKaiser", "StartAdvance", [])` and `sendEvent(260.0, "BerezovKGKaiser", "StartAdvance2", [])` - the second leg's delay accounts for the ~172-second transit time for the first leg (1725 units at 10 m/s) plus a buffer.

Verified balance (`MissionTasks.script`: 45/45, 109/109, 26/26; `Mission.script`: 13/13, 84/84, 33/33) and mirrored to the docs repo. This is a well-evidenced fix, not a blind guess - grounded in a clean visual ruling-out of the leading alternative hypothesis plus a concrete, checkable difference in the actual code. Not yet re-tested.
