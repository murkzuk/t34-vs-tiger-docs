...
**Fix**: renamed the object back to `"MainPlayerUnit"` in `Content.script` (only the ID changed - position/class/properties untouched). Synced to the docs repo. Not yet re-tested - needs another pass in the Editor to confirm the duplicate T-34/85 is gone.
