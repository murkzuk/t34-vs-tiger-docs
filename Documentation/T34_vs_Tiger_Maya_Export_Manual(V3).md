...
This manual (Phase 0 of the issue #12 effort) captures everything the plaintext MEL/tutorial sources reveal for free, with zero binary reverse-engineering. It does **not** itself reveal the actual `.ms2` byte-level format — that's Phase 1, now underway, see `Documentation/MS2_Binary_Format_Findings_2026-07-03.md` for real, verified progress (the full per-vertex position/normal/UV/triangle-index layout for a simple mesh is already decoded and cross-checked against known geometry). Recommended phases from here:
