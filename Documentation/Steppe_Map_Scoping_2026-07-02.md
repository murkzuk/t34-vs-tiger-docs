...
**Not yet done**: the generator still only jitters unit positions within the same small anchor cluster inherited from `Mission1` (a tiny fraction of the 18000x18000 map) - using more of the map's open space would need the RouterZone passability question above resolved first, or a different safe-zone-expansion approach. Not attempted this session.
